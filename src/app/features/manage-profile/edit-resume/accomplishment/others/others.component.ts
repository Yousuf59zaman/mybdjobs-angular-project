...
export class OthersComponent implements OnChanges {
  ...
  isOthersSectionOpen = input(false);
  isOtherNewFormOpen = signal(false);
  isOtherEditFormOpen = signal(false);
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (this.isOthersSectionOpen() && !this.isOpen()) {
  ...
  updateDateRangeString() {
    if (this.selectedStartDate && this.selectedEndDate) {
      ...
      this.otherForm.patchValue({ issueDate: diffDays.toString() });
  ...
  closeOtherForm(): void {
    this.isOtherNewFormOpen.set(false);
    this.editingSummary = null;
    this.otherForm.reset();
    this.dateRangeString = '';
    this.selectedStartDate = null;
  ...
  saveOtherSummary() {
    ...
    Object.keys(this.otherForm.controls).forEach(key => {
      const control = this.otherForm.get(key);
    ...
    this.accompolishmentService.insertUpdateInfo(command).subscribe({
      next: (response) => {
        ...
        if (successMsg) {
          ...
          this.toaster.show('Other accomplishment save failed!', {
            iconClass: 'lucide-check-circle',
  ...
  private getNextId(): number {
    return this.otherSummaries.length > 0
      ? Math.max(...this.otherSummaries.map(s => s.accomPlishmentId)) + 1
      : 1;
  }
  ...
  get issueDateControl(): FormControl {
    return this.otherForm.get('issueDate') as FormControl;
  }

  get urlControl(): FormControl {
    return this.otherForm.get('url') as FormControl;
  }

  get descriptionControl(): FormControl {
    return this.otherForm.get('description') as FormControl;
  }
